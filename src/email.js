...
export function buildAbandonedCartEmail({ customerName, items }) {
  // Encodes SKU:quantity pairs into the link — the cart page reads this
  // and adds the exact item(s) automatically on load, looking up the
  // rest of each product's details (name, image, price) itself. This
  // works regardless of what's already in the customer's browser,
  // since it's adding a known item fresh rather than trying to restore
  // whatever their cart looked like before.
  const addParam = items.map((i) => `${i.sku}:${i.quantity}`).join(',')
  const cartUrl = `https://hijabibridal.github.io/cart?add=${encodeURIComponent(addParam)}`
  ...
  return wrapEmail(`
    ...
    <p style="text-align: center; margin: 24px 0;">
      <a href="${cartUrl}" style="background: #db2777; color: #fff; padding: 12px 24px; border-radius: 999px; text-decoration: none; font-weight: bold; text-transform: uppercase; font-size: 13px;">
        Return to Cart
      </a>
    </p>
    <p>With love,<br/>Halal Nails</p>
